...
async function runMirror() {
  ...
  console.log(`✨ Found ${logs.length} new proposals`);
  ...
  for (const log of logs) {
    ...
    try {
      ...
      const endTs    = endBlk.timestamp - 24 * 60 * 60; // 24h before close
...
runMirror().catch(e => {
  console.error('mirror.js fatal error:', e);
  process.exit(1);
});